...
class TypeDictNodeEncoder(torch.nn.Module):
    def __init__(self, emb_dim):
        ...
        self.trunc_init = cfg.dataset.get('trunc_init', False)
        self.uniform_init = cfg.dataset.get('uniform_init', False)
    ...
    def init_weights(self):
        ...
        # self.encoder.apply(xavier_normal_init_)
        # nn.init.normal_(self.encoder.weight)
        # nn.init.uniform_(self.encoder.weight.data)
        if self.trunc_init:
            trunc_normal_(self.encoder.weight, 0, 0.02)
        elif self.uniform_init:
            nn.init.uniform_(self.encoder.weight, -1, 1)
        else:
