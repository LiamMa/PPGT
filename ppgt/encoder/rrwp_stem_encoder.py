...
from ..layer.utils.linear import AffineTransformLayer
from ..layer.utils.init import trunc_init_
...
class RRWPStemNodeEncoder(torch.nn.Module):
    ...
    def __init__(self, out_dim, use_bias=False, pe_name="rrwp"):
        ...
        self.pe_name = cfg.gt.stem.get('pe_name', pe_name)


        param = cfg.posenc_RRWP
        emb_dim = param.ksteps + 1
        if cfg.posenc_RRWF.enable:
            param = cfg.posenc_RRWF
            emb_dim = param.ksteps * (cfg.dataset.edge_encoder_num_types + 1)
        ...
        self.use_post_norm = cfg.gt.stem.get('post_norm', False)
        ...
        self.log_pe = cfg.gt.stem.get('log_pe', False)


        self.kp_enc = cfg.gt.stem.get('kp_encoding', False)
        if self.kp_enc:
            kp_dim = cfg.posenc_RRWP.kp_order *  (cfg.posenc_RRWP.ksteps + 1)
            self.kp_fc = nn.Linear(kp_dim, out_dim, bias=False)
    ...
    def forward(self, batch):
        ...
        out_val = batch[f"{self.pe_name}"]


        # ------------- Rescaling and LogPE -----------
        # if self.kernel_rescale:
        #     graph_order = (batch.ptr[1:] - batch.ptr[:-1]).view(-1, 1)
        #     # kernel_exp = scatter(out_val, out_idx[1], dim=0, dim_size=batch.num_nodes, reduce='mean')
        #     out_val = out_val * graph_order[batch.batch]

        # if self.log_pe:
            # out_val = torch.log(1 + out_val)

        # -----------------
        rrwp = self.pe_fc(self.raw_norm(out_val.flatten(1)))
        if self.sep_norm:
        ...
        if self.kp_enc:
            batch.x = batch.x + self.kp_fc(batch.kp_enc)

        batch.x = self.post_norm(x)
    ...
    def forward(self, batch):
        ...
        with torch.no_grad():
            ...
            out_idx, out_val = pyg.utils.coalesce(
                out_idx, out_val, num_nodes=batch.num_nodes, reduce='add'
            )
            #

            if self.kernel_rescale:
                kernel_exp = scatter(out_val, out_idx[1], dim=0, dim_size=batch.num_nodes, reduce='mean')
                kernel_exp[kernel_exp==0] = 1.
                out_val = out_val / kernel_exp[out_idx[1]]
