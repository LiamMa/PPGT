...
def extended_optim_cfg(cfg):
    ...
    cfg.optim.num_cycles = 0.5 # 1. also valid for timm?

    cfg.optim.min_lr_mode = "threshold"

    # Multiply all dropout / drop-path rates by this factor every ``dropout_decay_every`` epochs
    # (first decay at epoch L, 2L, …). Disabled when ``dropout_decay_factor`` is unset.
    cfg.optim.dropout_decay_factor = None
    cfg.optim.dropout_decay_every = 0
