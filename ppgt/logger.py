...
import ppgt.metrics_ogb as metrics_ogb
from ppgt.metric_wrapper import MetricWrapper
...
# Custom logger extending PyG's Logger to support various evaluation metrics and logging utilities.
class CustomLogger(Logger):
    # Initialize the custom logger with optional test scoring.
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # Whether to run comparison tests of alternative score implementations.
        self.test_scores = False

    # Gather basic statistics like loss, learning rate, parameter count, and iteration time.
    # Includes GPU memory usage if available.
    def basic(self):
        stats = {
    ...
    def classification_binary(self):
        ...
        if true.shape[0] < 1e7:  # AUROC computation for very large datasets is too slow.
            ...
            # For binary tasks GraphGym sets `dim_out = 1`, so `pred_score`
            # is 1-D and there is no class dimension to read.
            auroc_score = auroc(pred_score.to(torch.device(cfg.device)),
                                true.to(torch.device(cfg.device)),
                                num_classes=(pred_score.shape[1]
                                             if pred_score.ndim > 1 else None),
                                pos_label=1)
    ...
    def classification_multilabel(self):
        ...
        # islabeled_mask = true == true
        # true, pred_score = true[islabeled_mask], pred_score[islabeled_mask]
        # flatten true/pred and remove the potential 'nan' in true
        ...
        threshold = 0.  # threshold for binary classification



        # fixme: attempt add multilabel sigmoid
        apply_sigmoid = False
        if apply_sigmoid:
        ...
        if self.test_scores:
            ...
            ogb = {
                'accuracy': reformat(metrics_ogb.eval_acc(
                    true, (pred_score > threshold).astype(int))['acc']),
                'ap': reformat(metrics_ogb.eval_ap(true, pred_score)['ap']),
                'auc': reformat(
                    metrics_ogb.eval_rocauc(true, pred_score)['rocauc']),
            }
            assert np.isclose(ogb['accuracy'], results['accuracy'], atol=1e-5)
            assert np.isclose(ogb['ap'], results['ap'], atol=1e-5)
            assert np.isclose(ogb['auc'], results['auc'], atol=1e-5)
    ...
    def regression(self):
        ...
        return {
            ...
            'spearmanr': reformat(eval_spearmanr(true,
                                                 pred)['spearmanr']),
            'mse': reformat(mean_squared_error(true, pred)),
            # 'rmse': reformat(mean_squared_error(true, pred, squared=False))), # original implementation, has bug on latter sklearn
    ...
    def write_epoch(self, cur_epoch):
        ...
        eta_stats = {'eta': round(self.eta(cur_epoch), cfg.round),
                     'eta_hours': round(self.eta(cur_epoch) / 3600, cfg.round)} if cur_epoch >= 0 else {}
