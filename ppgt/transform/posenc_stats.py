...
from ..layer.utils.scatter import scatter_add
from functools import partial

from .rrwp import add_full_rrwp
from .I2subgraph import create_I2_subgraph
...
def compute_posenc_stats(data, pe_types, is_undirected, cfg):
    ...
    for t in pe_types:
        if t not in ['LapPE', 'EquivStableLapPE', 'SignNet',
                     'RWSE', 'HKdiagSE', 'HKfullPE', 'ElstaticSE',
                     'RRWP', 'RRWPv2', 'DFW', "Coord", 'RRWF', 'VN',
                     'ChebII', 'addX',
                     'SGRRWP', 'JRRWP', 'PathWalk',
                     'SubCount', 'PathCount',
                     'ESC', 'AddVN',
                     'I2SG', 'EgoSG', 'I2SE',
                     'CycleCount',
    ...
    # if 'AddCLS'  in pe_types:
    #     param = cfg.posenc_AddCLS
    #     data = add_class_token(data, **param)
    ...
    if 'RRWP' in pe_types:
        ...
        pe_func = add_full_rrwp
        transform = partial(pe_func,
    ...
    if 'I2SG' in pe_types:
        param = cfg.posenc_I2SG
        pe_func = create_I2_subgraph
        transform = partial(pe_func,
                            **param,
                            )
        data = transform(data)
    ...
    # if 'I2SE' in pe_types:
    #     param = cfg.posenc_I2SE
    #     pe_func = create_I2_SE
    #     transform = partial(pe_func,
    #                         **param,
    #                         )
    #     data = transform(data)
...
class ComputePosencStat(BaseTransform):
    ...
    # ---> modify for later version of PyG and PyTorch
    # @abstractmethod
