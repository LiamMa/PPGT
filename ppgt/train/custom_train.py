...
from ppgt.loss.subtoken_prediction_loss import subtoken_cross_entropy
from ppgt.utils import cfg_to_dict, flatten_dict, make_wandb_name, mlflow_log_cfgdict
from ppgt.network.base import apply_dropout_decay
...
def train_epoch(logger, loader, model, optimizer, scheduler, batch_accumulation, ema_model=None, rank=None):
    model.train()
    optimizer.zero_grad()
    time_start = time.time()

    if cfg.train.get('debug', False):
...
@torch.no_grad()
def eval_epoch(logger, loader, model, split='val', one_batch=False):
    model.eval()
    time_start = time.time()
    batch_id = 0
    for batch in loader:
...
def custom_train(loggers, loaders, model, optimizer, scheduler,
    ...
    Args:
        ...
        rank: process rank for multi-GPU (DDP) training; None for single device
        world_size: number of DDP processes; None for single device
    """
    # loggers[0].tb_writer.add_graph(model.model)

    cfg.vp_momentum = min(1., cfg.vp_momentum)
    ...
    if cfg.wandb.use:
        ...
        # An empty entity means "the default entity of the logged-in user".
        run = wandb.init(entity=cfg.wandb.entity or None,
    ...
    # ------------- EMA model ----------    
    ema_model = None
    ema = cfg.optim.get('EMA', False)
    ema_decay = cfg.optim.get('EMA_decay', 0.999)
    if ema:
    ...
    dropout_decay_factor = cfg.optim.get('dropout_decay_factor', None)

    for cur_epoch in range(start_epoch, cfg.optim.max_epoch):
        # cfg.cur_epoch = cur_epoch
        if rank is not None:
            loaders[0].sampler.set_epoch(cur_epoch)

        if dropout_decay_factor is not None and cur_epoch > 0:
            L = int(cfg.optim.get('dropout_decay_every', 0.))
            if L > 0 and cur_epoch % L == 0:
                f = float(dropout_decay_factor)
                apply_dropout_decay(model, f)
                # if ema_model is not None:
                #     apply_dropout_decay(ema_model, f)
                print(model)


        start_time = time.perf_counter()
        train_epoch(loggers[0], loaders[0], model, optimizer, scheduler,
                    cfg.optim.batch_accumulation, ema_model=ema_model, rank=rank)


        # ema_model will be None if not allowed
        perf[0].append(loggers[0].write_epoch(cur_epoch))


        # Every rank evaluates: the val/test loaders are not sharded, so all
        # ranks get the same metrics, and the LR scheduler below is driven by
        # `val_perf` -- skipping it on rank > 0 would leave that rank without a
        # validation metric and desynchronise its learning-rate schedule.
        if is_eval_epoch(cur_epoch):
            for i in range(1, num_splits):
                if ema_model is None:
                    eval_epoch(loggers[i], loaders[i], model,split=split_names[i - 1])
                else:
                    # Todo: probably need to collect BN statistics again for model with BN
                    eval_epoch(loggers[i], loaders[i], ema_model, split=split_names[i - 1])
        ...
        # DEBUG:
        if val_perf[-1]['loss'] > 0.6 and cur_epoch > 30:
            cfg.STOP = True
    ...
    load_ckpt(model, optimizer, scheduler, epoch=best_epoch)
    for i in range(0, num_splits):
        eval_epoch(loggers[i], loaders[i], model,split=split_names[i - 1])
        perf[i].append(loggers[i].write_epoch(-1))
...
def log_attn_weights(loggers, loaders, model, optimizer=None, scheduler=None):
    ...
    from ppgt.utils import unbatch, unbatch_edge_index
