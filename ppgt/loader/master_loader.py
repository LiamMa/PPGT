...
from torch_geometric.graphgym.loader import load_pyg, set_dataset_attr #, load_ogb
# from torch_geometric.graphgym.loader import load_ogb
from ppgt.loader.dataset.ogb import load_ogb # the default load_ogb will automatically to_undirected

from ppgt.loader.dataset.aqsol_molecules import AQSOL
from ppgt.loader.dataset.coco_superpixels import COCOSuperpixels
from ppgt.loader.dataset.malnet_tiny import MalNetTiny
from ppgt.loader.dataset.voc_superpixels import VOCSuperpixels
from ppgt.loader.split_generator import (prepare_splits,
                                        set_dataset_splits)
from ppgt.transform.posenc_stats import compute_posenc_stats, ComputePosencStat
from ppgt.transform.transforms import (pre_transform_in_memory,
                                      typecast_x, concat_x_and_pos,
                                      clip_graphs_to_size)

from ppgt.loader.node2subgraph_loader import node2subgraph  # , NodeToSubgraphDataset
from .dataset.BRECDataset_v3 import BRECDataset
...
def log_loaded_dataset(dataset, format, name):
    ...
    if hasattr(dataset.data, 'sparsity'):
        logging.info(f"avg_sparsity: train={cfg.dataset.avg_sparsity_train}, val={cfg.dataset.avg_sparsity_val}, test={cfg.dataset.avg_sparsity_test}")
...
def load_dataset_master(format, name, dataset_dir):
    ...
    elif format.startswith('BREC'):
        dataset = BRECDataset(name='BREC', root=osp.join(dataset_dir, 'BREC'))
        cfg.dataset.task = 'bypass'
        dataset.data['train_graph_index'] = torch.arange(len(dataset))
        # to bypass the default splitting checking
    else:
    ...
    if cfg.train.sampler == 'node2subgraph':
        dataset = convert_node_to_subgraphs(dataset)
    ...
    if pe_enabled_list:
        ...
        if cfg.dataset.pe_transform_on_the_fly:
            # set pe_transform as pre_transform of dataloader (compute on the fly for each mini-batch)
            warnings.warn('PE transform on the fly to save memory consumption; experimental, please only use for RRWP')
        ...
        elif cfg.dataset.brec_pe_transform:
            # special case for BREC dataset, compute for each training example in the pipeline
            warnings.warn('PE transform on the fly to save memory consumption; experimental, please only use for RWSE/RWPSE')
            pe_transform_fn = partial(pre_transform_in_memory,
                                      transform_func=partial(compute_posenc_stats,
            ...
            # cache pe_transform for separate ussage
            register_loader('pe_transform', pe_transform_fn)
        elif cfg.dataset.get('pe_n2s_pre_compute', False):
            pass
        else:
            # (default) compute PE and store in memory (in dataset)
    ...
    if 'train_graph_index' in dataset.data:
        # compute statistics on training set; bypass if not specified.
        average_num_nodes(dataset[dataset.data['train_graph_index']])
        average_sparsity(dataset)
...
def average_sparsity(dataset):
    '''
        Compute average number of nodes per graph in training set
    '''
    if 'sparsity' not in dataset.data:
        return None

    cfg.dataset.avg_sparsity_train = torch.mean(dataset[dataset.data['train_graph_index']].data.sparsity).item()
    cfg.dataset.avg_sparsity_val= torch.mean(dataset[dataset.data['val_graph_index']].data.sparsity).item()
    cfg.dataset.avg_sparsity_test = torch.mean(dataset[dataset.data['test_graph_index']].data.sparsity).item()
    return (cfg.dataset.avg_sparsity_train, cfg.dataset.avg_sparsity_val, cfg.dataset.avg_sparsity_test)
#
...
def preformat_OGB_Graph(dataset_dir, name):
    ...
    if name == 'ogbg-ppa':
        ...
        from ppgt.loader.ogbg_code2_utils import idx2vocab, \
...
def preformat_PCQM4Mv2Contact(dataset_dir, name):
    ...
    try:
        # Load locally to avoid RDKit dependency until necessary
        from ppgt.loader.dataset.pcqm4mv2_contact import \
...
def preformat_Peptides(dataset_dir, name):
    ...
    try:
        # Load locally to avoid RDKit dependency until necessary.
        from ppgt.loader.dataset.peptides_functional import \
            PeptidesFunctionalDataset
        from ppgt.loader.dataset.peptides_structural import \
...
def preformat_AQSOL(dataset_dir, name=None):
...
def convert_node_to_subgraphs(dataset):
        hops = int(cfg.train.get('k_hop_subgraph', 2))
        max_nodes = int(cfg.train.get('max_nodes', 250))
        num_workers = int(cfg.train.get('num_workers', cfg.num_workers))
        global_rrwp_pre_compute = cfg.dataset.get("pe_n2s_pre_compute", False)
        train_dataset, val_dataset, test_dataset = node2subgraph(dataset, max_hops=hops, num_workers=num_workers, max_nodes=max_nodes, pre_compute_rrwp=global_rrwp_pre_compute, debug=False)
        ...
        # ]
        # splits = {"train_graph_index": 0, "val_graph_index": 1, "test_graph_index": 2}
        # # dataset = dotdict({0: train_dataset, 1: val_dataset, 2: test_dataset, "data": splits})

        dataset = join_dataset_splits([train_dataset, val_dataset, test_dataset])
